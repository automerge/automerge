...
#[no_mangle]
pub unsafe extern "C" fn AMputActor<'a>(
    doc: *mut AMdoc,
    value: *const u8,
    count: usize,
) -> *mut AMresult<'a> {
...
#[no_mangle]
pub unsafe extern "C" fn AMputActorHex<'a>(
    doc: *mut AMdoc,
...
pub unsafe extern "C" fn AMmapPutInt<'a>(
    ...
    let doc = to_doc!(doc);
    to_result(doc.set(to_obj_id!(obj_id), to_str(key), value))
}
...
pub unsafe extern "C" fn AMmapPutStr<'a>(
    ...
    let doc = to_doc!(doc);
    to_result(doc.set(to_obj_id!(obj_id), to_str(key), to_str(value)))
}
...
pub unsafe extern "C" fn AMmapPutBytes<'a>(
    ...
    to_result(doc.set(to_obj_id!(obj_id), to_str(key), vec))
}
...
pub unsafe extern "C" fn AMmapPutNull<'a>(
    ...
    let doc = to_doc!(doc);
    to_result(doc.set(to_obj_id!(obj_id), to_str(key), ()))
}
...
pub unsafe extern "C" fn AMmapPutObject<'a>(
    ...
    let doc = to_doc!(doc);
    to_result(doc.set_object(to_obj_id!(obj_id), to_str(key), obj_type.into()))
}
