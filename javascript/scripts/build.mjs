...
function isStep(stepStr) {
    return ["all", "build-wasm", "build-wasm-tarball", "compile-typescript", "transpile-cjs"].includes(stepStr)
}
...
function buildWasm(outputDir) {
    ...
    execSync("cargo build --release --target wasm32-unknown-unknown --profile release", {
        cwd: automergeWasmPath
    })

    const wasmBlobPath = path.join(rustProjectDir, "target", "wasm32-unknown-unknown", "release", "automerge_wasm.wasm")
    ...
    */
    function runWasmBindgen(target) {
        ...
        execSync(`wasm-bindgen ${wasmBlobPath} --out-dir ${outputPath} --target ${target} --no-typescript --weak-refs`, {
            cwd: __dirname
        })
    }
...
function buildWasmTarball(outputLocation) {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "automerge-wasm-tarball-"))
    buildWasm(outputDir)
...
function copyAndFixupWasm(wasmBuildTarball) {
    const automergeWasmPath = path.join(__dirname, "..", "..","rust", "automerge-wasm")
    ...
    console.log("renaming 'automerge_wasm.js' to 'automerge_wasm.cjs' in the node target")
    const nodeOutputPath = path.join(jsProjectDir, "src", "wasm_bindgen_output", "nodejs")
    fs.cpSync(path.join(nodeOutputPath, "automerge_wasm.js"), path.join(nodeOutputPath, "automerge_wasm.cjs"))


    console.log("encoding the 'wasm' blob in the 'web' target into a base64 string")
    const webOutputPath = path.join(jsProjectDir, "src", "wasm_bindgen_output", "web")
    ...
    const wasmBlobBase64Path = path.join(jsProjectDir, "src", "wasm_bindgen_output", "web", "automerge_wasm_bg_base64.js")
    fs.writeFileSync(wasmBlobBase64Path, `export const wasmBlobBase64 = "${wasmBlobBase64}"`)


    console.log("writing a shim to load the base64 encoded wasm in the 'web' target")
    const wasmBlobBase64ShimPath = path.join(jsProjectDir, "src", "wasm_bindgen_output", "web", "index.js")
    fs.writeFileSync(wasmBlobBase64ShimPath, `
    ...
    fs.copyFileSync(path.join(automergeWasmPath, "index.d.ts"), path.join(jsProjectDir, "src", "wasm_types.d.ts"))
}
...
function compileTypescript() {
    ...
    execSync("node_modules/.bin/tsc -p config/declonly.json --emitDeclarationOnly", {
        cwd: jsProjectDir,
    })

    const mjsDir = path.join(jsProjectDir, "dist", "mjs")
    const wasmBindgenSrcDir = path.join(jsProjectDir, "src", "wasm_bindgen_output")
    const mjsWasmDir = path.join(mjsDir, "wasm_bindgen_output")

    console.log("copying wasm_bindgen_output directory to dist/mjs/wasm_bindgen_output")
    fs.mkdirSync(mjsWasmDir, { recursive: true })
    fs.cpSync(wasmBindgenSrcDir, mjsWasmDir, { recursive: true })

    fs.copyFileSync(path.join(jsProjectDir, "src", "wasm_types.d.ts"), path.join(jsProjectDir, "dist", "wasm_types.d.ts"))
}
...
if (step === "all" || step === "transpile-cjs") {
    ...
    const wasmBindgenSrc = path.join(jsProjectDir, "/src/wasm_bindgen_output/nodejs/automerge_wasm_bg.wasm")
    const cjsDir = path.join(jsProjectDir, "/dist/cjs")
    fs.copyFileSync(wasmBindgenSrc, path.join(cjsDir, "automerge_wasm_bg.wasm"))

    fs.copyFileSync(path.join(jsProjectDir, "/src/wasm_types.d.ts"), path.join(cjsDir, "wasm_types.d.ts"))
}
