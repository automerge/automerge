...
impl TryFrom<JS> for Vec<ChangeHash> {
    ...
    fn try_from(value: JS) -> Result<Self, Self::Error> {
        ...
        let value: Result<Vec<ChangeHash>, _> = value.iter().map(|j| serde_wasm_bindgen::from_value(j)).collect();
...
pub(crate) fn get_heads(heads: Option<Array>) -> Option<Vec<ChangeHash>> {
    let heads = heads?;
    let heads: Result<Vec<ChangeHash>, _> = heads.iter().map(|j| serde_wasm_bindgen::from_value(j)).collect();
    heads.ok()
}
