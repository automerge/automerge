...
pub fn examine(mut input: impl std::io::Read, mut output: impl std::io::Write, is_tty: bool) -> Result<(), ExamineError> {
    ...
    let uncompressed_changes: Vec<UncompressedChange> = changes.iter().map(|c| c.decode()).collect();
    if is_tty {
        ...
        output.write_all(&json_changes.into_bytes()).map_err(|e| ExamineError::ErrWritingToOutput{source: e})?;
    }
    Ok(())
}
