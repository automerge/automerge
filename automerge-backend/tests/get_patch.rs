...
fn test_include_most_recent_value_for_key() {
    ...
    let expected_patch = Patch {
        ...
        diffs: RootDiff {
            props: hashmap! {
                "bird".into() => hashmap!{
                    actor.op_id_at(NonZeroU64::new(2).unwrap()) => Diff::Value("blackbird".into()),
                }
            },
        },
    };
...
fn test_includes_conflicting_values_for_key() {
    ...
    let expected_patch = Patch {
        ...
        diffs: RootDiff {
            props: hashmap! {
                "bird".into() => hashmap!{
                    actor1.op_id_at(NonZeroU64::new(1).unwrap()) => Diff::Value("magpie".into()),
                    actor2.op_id_at(NonZeroU64::new(1).unwrap()) => Diff::Value("blackbird".into()),
                },
            },
        },
    };
        ...
        diffs: RootDiff {
            props: hashmap! {
                "counter".into() => hashmap!{
                    actor.op_id_at(NonZeroU64::new(1).unwrap()) => Diff::Value(ScalarValue::Counter(3))
                }
            },
        },
    };
                    ...
                    actor.op_id_at(NonZeroU64::new(1).unwrap()) => Diff::Map(MapDiff{
                        ...
                        props: hashmap!{
                            "sparrows".into() => hashmap!{
                                actor.op_id_at(NonZeroU64::new(4).unwrap()) => Diff::Value(ScalarValue::F64(15.0))
                            }
                        }
                    })
                }
...
fn test_includes_latests_state_of_list() {
    ...
    let change1: Change = amp::Change {
        ...
        operations: vec![
            ...
            Op {
                obj: ObjectId::from(actor.op_id_at(NonZeroU64::new(2).unwrap())),
                    ...
                    actor.op_id_at(NonZeroU64::new(1).unwrap()) => Diff::List(ListDiff{
                        ...
                        edits: vec![DiffEdit::SingleElementInsert{
                            ...
                            value: Diff::Map(MapDiff{
                                ...
                                props: hashmap!{
                                    "title".into() => hashmap!{
                                        actor.op_id_at(NonZeroU64::new(3).unwrap()) => Diff::Value("water plants".into()),
                                    },
                                    "done".into() => hashmap!{
                                        actor.op_id_at(NonZeroU64::new(4).unwrap()) => Diff::Value(false.into())
                                    }
                                }
                            })
                        }],
...
fn test_includes_date_objects_at_root() {
    ...
    let expected_patch = Patch {
        ...
        diffs: RootDiff {
            props: hashmap! {
                "now".into() => hashmap!{
                    actor.op_id_at(NonZeroU64::new(1).unwrap()) => Diff::Value(ScalarValue::Timestamp(1_586_541_033_457))
                }
            },
        },
    };
...
fn test_includes_updates_for_conflicting_list_elements() {
    ...
    let local_change = amp::Change {
        ...
        operations: vec![
            ...
            Op {
                ...
                obj: local_actor.op_id_at(NonZeroU64::new(1).unwrap()).into(),
    ...
    let remote_change_1: Change = amp::Change {
        ...
        operations: vec![Op {
            ...
            obj: local_actor.op_id_at(NonZeroU64::new(1).unwrap()).into(),
            key: local_actor.op_id_at(NonZeroU64::new(2).unwrap()).into(),
            pred: vec![local_actor.op_id_at(NonZeroU64::new(2).unwrap())].into(),
            insert: false,
        }],
    }
    .try_into()
    ...
    let expected_patch = Patch {
        clock: hashmap! {
            local_actor.clone() => NonZeroU64::new(1).unwrap(),
            actor1.clone() => NonZeroU64::new(1).unwrap(),
            actor2.clone() => NonZeroU64::new(1).unwrap(),
        },
        max_op: NonZeroU64::new(2),
        ...
        diffs: RootDiff {
            props: hashmap! {
                "list".into() => hashmap!{
                    local_actor.op_id_at(NonZeroU64::new(1).unwrap()) => Diff::List(ListDiff{
                        object_id: ObjectId::from(local_actor.op_id_at(NonZeroU64::new(1).unwrap())),
                        edits: vec![
                            DiffEdit::SingleElementInsert{
                                index: 0,
                                elem_id: local_actor.op_id_at(NonZeroU64::new(2).unwrap()).into(),
                                op_id: actor1.op_id_at(NonZeroU64::new(1).unwrap()),
                                value: Diff::Value("remote1".into()),
                            },
                            DiffEdit::Update{
                                index: 0,
                                op_id: actor2.op_id_at(NonZeroU64::new(1).unwrap()),
