...
impl Edits {
    ...
    pub(super) fn append_edit(&mut self, mut edit: amp::DiffEdit) {
        if let Some(mut last) = self.0.last_mut() {
            match (&mut last, &mut edit) {
                (
                    ...
                    let op_id = elem_id.as_opid().unwrap();
                    if *index + 1 == *next_index && op_id.delta(next_elem_id.as_opid().unwrap(), 1)
                    {
                        *last = amp::DiffEdit::MultiElementInsert {
                            index: *index,
                            first_opid: op_id.clone(),
                            values: vec![
                                mem::replace(value, amp::ScalarValue::Null),
                                mem::replace(next_value, amp::ScalarValue::Null),
                            ],
                        };
                    } else {
                        self.0.push(edit)
                ...
                (
                    ...
                    if *index + (values.len() as u64) == *next_index
                        && first_opid.delta(elem_id.as_opid().unwrap(), values.len() as u64)
                    {
                        values.push(mem::replace(value, amp::ScalarValue::Null));
                    } else {
                        self.0.push(edit)
                    }
                }
