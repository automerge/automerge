    ...
    V: Clone + Debug + PartialEq
{
    ...
    fn remove_after(&mut self, from_level: usize, removed_level: usize, links: &Vec<Link<K>>) {
        for level in from_level..self.level {
            if level < removed_level {
                self.next[level] = links[level].clone()
            } else {
    ...
    fn remove_before(&mut self, from_level: usize, removed_level: usize, links: &Vec<Link<K>>) {
        for level in from_level..self.level {
            if level < removed_level {
                self.prev[level] = links[level].clone();
            } else {
    ...
    fn insert_after(&mut self, new_key: &Option<K>, new_level: usize, from_level: usize, distance: usize) -> Result<(),AutomergeError> {
        if new_level > self.level && !self.is_head() {
            Err(AutomergeError::SkipListError("Cannot increase the level of a non-head node".to_string()))
        } else {
            self.level = max(self.level, new_level);

            for level in from_level..self.level {
                if level < new_level {
                    let link = Link { key: new_key.clone(), count: distance };
    ...
    fn insert_before(&mut self, new_key: &Option<K>, new_level: usize, from_level: usize, distance: usize) -> Result<(),AutomergeError> {
        if new_level > self.level {
            Err(AutomergeError::SkipListError("Cannot increase the level on insert-before".to_string()))
        } else {
            for level in from_level..self.level {
                if level < new_level {
                    self.prev[level] = Link { key: new_key.clone(), count: distance };
                } else {
                    self.prev[level].count += 1;
                }
            }
...
pub (crate) struct SkipList<K,V> 
    where K: Clone + Debug + Hash + PartialEq + Eq,
    V: Clone + Debug + PartialEq
{
    nodes: HashMap<Option<K>,Node<K,V>>,
    rng: ThreadRng,
    pub len: usize,
}
    ...
    V: Clone + Debug + PartialEq
{
    pub fn new() -> SkipList<K,V> {
        ...
        nodes.insert(None,head);
        let len = 0;
        let rng = rand::thread_rng();
        SkipList { nodes, len, rng }
    }
    ...
    pub fn insert_index(&mut self, index: isize, key: K, value: V) -> Result<(),AutomergeError> {
        if index == 0 {
            ...
            let suc = self.key_of(index - 1).ok_or(AutomergeError::SkipListError("Insert index out of bounds".to_string()))?;
            self.insert_after(&suc, key, value)
        }
    }

    pub fn remove_index(&mut self, index: isize) -> Result<(),AutomergeError> {
        let key = self.key_of(index).ok_or(AutomergeError::SkipListError("Remove index out of bounds".to_string()))?;
        self.remove_key(&key)
    }
    ...
    fn get(&self, key: &K) -> Option<&V> {
        self.get_node(&Some(key.clone())).ok().and_then(|n| n.value.as_ref())
    }

    fn get_node(&self, key: &Option<K>) -> Result<&Node<K,V>,AutomergeError> {
        self.nodes.get(key).ok_or(AutomergeError::SkipListError("Key not found".to_string()))
    }

    fn get_node_mut(&mut self, key: &Option<K>) -> Result<&mut Node<K,V>,AutomergeError> {
        self.nodes.get_mut(key).ok_or(AutomergeError::SkipListError("Key not found".to_string()))
    }

    fn predecessors(&self, predecessor: &Option<K>, max_level: usize) -> Result<Vec<Link<K>>,AutomergeError> {
        let mut pre = vec![Link { key: predecessor.clone(), count: 1 }];
        ...
        for level in 1..max_level {
            ...
            while link.key.is_some() {
                ...
                if node.level < level { return Err(AutomergeError::SkipListError("Level lower than expected".to_string())) }
    ...
    fn successors(&self, successor: &Option<K>, max_level: usize) -> Result<Vec<Link<K>>,AutomergeError> {
        let mut suc = vec![Link { key: successor.clone(), count: 1 }];
    ...
    pub fn remove_key(&mut self, key: &K) -> Result<(),AutomergeError> {
        let removed = self.nodes.remove(&Some(key.clone())).ok_or(AutomergeError::SkipListError("The given key cannot be removed because it does not exist".to_string()))?;
        ...
        for level in 1..(max_level + 1) {
            ...
            if level == max_level || pre.get(level).map(|l| &l.key) != pre.get(pre_level).map(|l| &l.key) {
                self.get_node_mut(&pre[pre_level].key)?.remove_after(pre_level, update_level, &suc);
                pre_level = level;
            }
            if suc[suc_level].key.is_some() && (level == max_level || suc.get(level).map(|l| &l.key) != suc.get(suc_level).map(|l| &l.key)) {
                self.get_node_mut(&suc[suc_level].key)?.remove_before(suc_level, update_level, &pre);
                suc_level = level;
            }
        }
        Ok(())
    ...
    pub fn key_of(&self, mut index: isize) -> Option<K> {
        ...
        loop {
            if count == index + 1 {
                break;
            } else if count + node.next[level].count > index + 1 {
                level -= 1
            } else {
                count += node.next[level].count;
                node = &self.nodes.get(&node.next[level].key).unwrap() // correct to panic here
            }
        }
        node.key.clone()
    }
    ...
    pub fn insert_after(&mut self, predecessor: &K, key: K, value: V) -> Result<(),AutomergeError> {
        self._insert_after(&Some(predecessor.clone()),key,value)
    }

    fn _insert_after(&mut self, predecessor: &Option<K>, key: K, value: V) -> Result<(),AutomergeError> {
        let key = Some(key);
        ...
        for level in 1..(max_level+1) {
            ...
            if level == max_level || pre.get(level).map(|l| &l.key) != pre.get(pre_level).map(|l| &l.key) {
                self.get_node_mut(&pre[pre_level].key)?.insert_after(&key, update_level, pre_level, pre[pre_level].count)?;
                pre_level = level;
            }
            if suc[suc_level].key.is_some() && (level == max_level || suc.get(level).map(|l| &l.key) != suc.get(suc_level).map(|l| &l.key)) {
                self.get_node_mut(&suc[suc_level].key)?.insert_before(&key, update_level, suc_level, suc[suc_level].count)?;
                suc_level = level;
            }
        }
        ...
        self.nodes.insert(key.clone(), Node { key, value: Some(value), level: new_level, prev: pre, next: suc });
        Ok(())
    }

    pub fn index_of(&self, key: &K) -> Option<usize> {
       let key = Some(key.clone());
       if !self.nodes.contains_key(&key) { return None; }
...
{
    id: &'a Option<K>,
    nodes: &'a HashMap<Option<K>,Node<K,V>>,
}

impl<'a,K,V> Iterator for SkipIterator<'a,K,V> 
    where K: Debug + Clone + Hash + PartialEq + Eq,
    V: Debug + Clone + PartialEq
{
    type Item = &'a V;
    ...
    fn next(&mut self) -> Option<&'a V> {
        if self.id.is_none() {
            None
        } else if let Some(node) = self.nodes.get(self.id) {
