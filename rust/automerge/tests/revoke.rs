...
fn revocation_mask_survives_actor_reordering() {
    ...
    doc.revoke(bad.clone(), &pre_change);
...
#[test]
fn revoke_author_with_new_actor_hidden() {
    let good = Author::try_from("aaaa").unwrap();
    let bad = Author::try_from("bbbb").unwrap();
    let mut doc = AutoCommit::new().with_author(Some(bad.clone()));
    doc.put(ROOT, "foo", "bar").unwrap();

    let mut doc2 = doc.fork().with_author(Some(good));
    doc2.revoke(bad.clone(), &doc.get_heads());

    let mut doc3 = doc.fork().with_author(Some(bad));
    doc3.put(ROOT, "foo", "baz").unwrap();

    doc2.merge(&mut doc3).unwrap();
    assert_eq!(doc2.get(ROOT, "foo").unwrap().unwrap().0, "bar".into());
}
