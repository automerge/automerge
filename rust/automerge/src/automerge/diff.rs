...
fn log_list_diff<'a, I: Iterator<Item = Patch<'a>>>(
    ...
    patches.fold(0, |index, patch| match patch {
        ...
        Patch::Delete(before, id) => {
            patch_log.delete_seq(obj.id, id, index, 1, before.op.as_str().into(), false);
            index
        }
    });
}
...
fn log_text_diff<'a, I: Iterator<Item = Patch<'a>>>(
    ...
    patches.fold(0, |index, patch| match &patch {
        Patch::New(winner, marks) => {
            let by = *winner.op.id();
            if winner.op.is_put() {
                patch_log.splice2(obj.id,  by, index, winner.op.as_str(), marks.clone());
            } else {
                // blocks
                let value = winner.op.value_at(Some(winner.clock)).into();
                let id = *winner.op.id();
        ...
        Patch::Update {
            ...
            let by = *after.op.id();
            patch_log.splice2(obj.id, by, index, after.op.as_str(), marks.clone());
...
fn log_map_diff<'a, I: Iterator<Item = Patch<'a>>>(
    ...
    diffs
        ...
        .for_each(|(key, patch)| match patch {
            ...
            Patch::Delete(_, id) => patch_log.delete_map(obj.id, id, key),
        });
}
