...
use crate::patches::{InsertArgs, PutSeqArgs, TextRepresentation};
use crate::{
    exid::ExId,
    iter::{Keys, ListRange, MapRange, Values},
    marks::{Mark, RichText, RichTextStateMachine},
    patches::PatchLog,
...
fn log_list_diff<'a, I: Iterator<Item = Patch<'a>>>(
    ...
    patches.fold(0, |index, patch| match patch {
        ...
        Patch::Delete(before) => {
            patch_log.delete_seq(*obj, index, 1, before.op.block_id());
            index
        }
    });
}
...
fn log_text_diff<'a, I: Iterator<Item = Patch<'a>>>(
    ...
    patches.fold(0, |index, patch| match &patch {
        ...
        Patch::Update {
            ...
            patch_log.delete_seq(*obj, index, before.op.width(encoding), before.op.block_id());
