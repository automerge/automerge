...
use js_sys::{Array, Function, JsString, Object, Reflect, Symbol, Uint8Array, WeakMap};
...
pub(crate) struct ExportCache<'a> {
    objs: HashMap<ObjId, CachedObject, FxBuildHasher>,
    objs2: HashMap<ObjId, COWJsObj, FxBuildHasher>,
    metadata: WeakMap,
    user_data: JsValue,
...
impl<'a> ExportCache<'a> {
    ...
    #[inline(never)]
    pub(crate) fn wrap_scalar(&self, value: JsValue, datatype: Datatype) -> Result<JsValue, error::Export> {
    ...
    pub(crate) fn copy_on_write(
        ...
        match self.objs2.entry(obj.clone()) {
    ...
    */
    ...
    pub(crate) fn decode_obj_metadata(state: &JsValue) -> Option<ObjMetadata> {
        ...
        let mut bytes = array.get(0).dyn_into::<Uint8Array>();
