...
#[divan::bench_group(name = "splice_small_insert")]
mod splice_small_insert {
    use super::*;

    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v0_insert_1(bencher: Bencher) {
        let mut col = build_v0(10_000);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.splice(pos, 0, [rand_u64()]);
        });
    }

    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v1_insert_1(bencher: Bencher) {
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v0_insert_5(bencher: Bencher) {
        let mut col = build_v0(10_000);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.splice(pos, 0, rand_vals(5));
        });
    }

    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v1_insert_5(bencher: Bencher) {
        let mut col = build_v1(10_000);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.splice(pos, 0, rand_vals(5));
...
// ── Small splice: replace ───────────────────────────────────────────────────

#[divan::bench_group(name = "splice_small_replace")]
mod splice_small_replace {
    use super::*;
...
// ── Small splice: u64 replace (already above), now String insert/replace ────
...
#[divan::bench_group(name = "string_insert_small")]
mod string_insert_small {
    use super::*;

    #[divan::bench(args = [8, 64, 1024], max_time = Duration::from_secs(3))]
    fn v0_insert_1(bencher: Bencher, slen: usize) {
        let mut col = build_v0_str(10_000, slen);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.splice(pos, 0, [rand_string(slen)]);
        });
    }

    #[divan::bench(args = [8, 64, 1024], max_time = Duration::from_secs(3))]
    fn v1_insert_1(bencher: Bencher, slen: usize) {
        let mut col = build_v1_str(10_000, slen);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.insert(pos, rand_string(slen));
        });
    }

    #[divan::bench(args = [8, 64, 1024], max_time = Duration::from_secs(3))]
    fn v0_insert_5(bencher: Bencher, slen: usize) {
        let mut col = build_v0_str(10_000, slen);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.splice(pos, 0, rand_strings(5, slen));
        });
    }

    #[divan::bench(args = [8, 64, 1024], max_time = Duration::from_secs(3))]
    fn v1_insert_5(bencher: Bencher, slen: usize) {
        let mut col = build_v1_str(10_000, slen);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.splice(pos, 0, rand_strings(5, slen));
...
#[divan::bench_group(name = "string_replace_small")]
mod string_replace_small {
    use super::*;

    #[divan::bench(args = [8, 64, 1024], max_time = Duration::from_secs(3))]
    fn v0_replace_1(bencher: Bencher, slen: usize) {
        let mut col = build_v0_str(10_000, slen);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.splice(pos, 1, [rand_string(slen)]);
        });
    }

    #[divan::bench(args = [8, 64, 1024], max_time = Duration::from_secs(3))]
    fn v1_replace_1(bencher: Bencher, slen: usize) {
        let mut col = build_v1_str(10_000, slen);
        bencher.bench_local(|| {
            let pos = rand_usize() % col.len();
            col.splice(pos, 1, [rand_string(slen)]);
    ...
    #[divan::bench(args = [8, 64, 1024], max_time = Duration::from_secs(3))]
    fn v0_replace_5(bencher: Bencher, slen: usize) {
        let mut col = build_v0_str(10_000, slen);
        bencher.bench_local(|| {
            let len = col.len();
            if len < 6 { return; }
    ...
    #[divan::bench(args = [8, 64, 1024], max_time = Duration::from_secs(3))]
    fn v1_replace_5(bencher: Bencher, slen: usize) {
        let mut col = build_v1_str(10_000, slen);
        bencher.bench_local(|| {
            let len = col.len();
            if len < 6 { return; }
...
// ── advance_total vs advance_acc_by: random ────────────────────────────────

#[divan::bench_group(name = "advance_total_random")]
mod advance_total_random {
    use super::*;

    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v0_advance_acc_10k(bencher: Bencher) {
        let col = build_v0_prefix(10_000);
        // Advance by ~half the total acc
        let half = col.acc().as_u64() / 2;
        bencher.bench_local(|| {
            let mut iter = col.iter();
            let consumed = iter.advance_acc_by(half);
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v1_advance_total_10k(bencher: Bencher) {
        let col = build_v1_prefix(10_000);
        let half = col.get_prefix(col.len()) / 2;
        bencher.bench_local(|| {
            let mut iter = col.iter();
            let r = iter.advance_total(half);
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v0_advance_acc_100k(bencher: Bencher) {
        let col = build_v0_prefix(100_000);
        let half = col.acc().as_u64() / 2;
        bencher.bench_local(|| {
            let mut iter = col.iter();
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v1_advance_total_100k(bencher: Bencher) {
        let col = build_v1_prefix(100_000);
        let half = col.get_prefix(col.len()) / 2;
        bencher.bench_local(|| {
            let mut iter = col.iter();
...
// ── advance_total vs advance_acc_by: alternating runs ──────────────────────

#[divan::bench_group(name = "advance_total_runs")]
mod advance_total_runs {
    use super::*;
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v0_advance_acc_quarter(bencher: Bencher) {
        let col = build_v0_prefix_alternating(RUN_LEN, NUM_RUNS);
        let quarter = col.acc().as_u64() / 4;
        bencher.bench_local(|| {
            let mut iter = col.iter();
            let consumed = iter.advance_acc_by(quarter);
            std::hint::black_box(consumed)
        });
    }
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v1_advance_total_quarter(bencher: Bencher) {
        let col = build_v1_prefix_alternating(RUN_LEN, NUM_RUNS);
        let quarter = col.get_prefix(col.len()) / 4;
        bencher.bench_local(|| {
            let mut iter = col.iter();
            let r = iter.advance_total(quarter);
            std::hint::black_box(r)
        });
    }
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v0_advance_acc_half(bencher: Bencher) {
        let col = build_v0_prefix_alternating(RUN_LEN, NUM_RUNS);
        let half = col.acc().as_u64() / 2;
        bencher.bench_local(|| {
            let mut iter = col.iter();
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v1_advance_total_half(bencher: Bencher) {
        let col = build_v1_prefix_alternating(RUN_LEN, NUM_RUNS);
        let half = col.get_prefix(col.len()) / 2;
        bencher.bench_local(|| {
            let mut iter = col.iter();
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v0_advance_acc_3quarter(bencher: Bencher) {
        let col = build_v0_prefix_alternating(RUN_LEN, NUM_RUNS);
        let three_q = col.acc().as_u64() * 3 / 4;
        bencher.bench_local(|| {
            let mut iter = col.iter();
            let consumed = iter.advance_acc_by(three_q);
            std::hint::black_box(consumed)
        });
    }
    ...
    #[inline(never)]
    #[divan::bench(max_time = Duration::from_secs(3))]
    fn v1_advance_total_3quarter(bencher: Bencher) {
        let col = build_v1_prefix_alternating(RUN_LEN, NUM_RUNS);
        let three_q = col.get_prefix(col.len()) * 3 / 4;
        bencher.bench_local(|| {
            let mut iter = col.iter();
            let r = iter.advance_total(three_q);
            std::hint::black_box(r)
        });
    }
}
