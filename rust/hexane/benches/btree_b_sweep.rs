    ...
    b.counter(ItemsCount::new((OPS * K) as u64))
        .bench_local(|| {
            ...
            for _ in 0..OPS {
                ...
                let new: Vec<u64> = (0..K).map(|_| rng.random_range(0..N as u64)).collect();
                c.splice(pos, 0, new);
            }
            divan::black_box(c)
        });
...
fn agg_insert(b: Bencher) {
    ...
    b.counter(ItemsCount::new((OPS * K) as u64))
        .bench_local(|| {
            ...
            for _ in 0..OPS {
                let pos = rng.random_range(0..=c.len());
                let new: Vec<i64> = (0..K).map(|_| rng.random_range(0..N as i64)).collect();
                c.splice(pos, 0, new);
            }
            divan::black_box(c)
        });
...
fn agg_find_by_value(b: Bencher) {
    ...
    b.counter(ItemsCount::new(OPS as u64)).bench_local(|| {
        ...
        let mut acc: usize = 0;
        for _ in 0..OPS {
            let target = rng.random_range(0..N as i64);
