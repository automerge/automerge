...
fn main() {
    ...
    eprintln!("{:>10} {:>12} {:>12} {:>8}", "dist", "leb128", "bijou64", "ratio");
    for dist in DISTS {
    ...
    for dist in DISTS {
        ...
        eprintln!("{:>10} {:>12} {:>12} {:>7.3}", dist, l, b, b as f64 / l as f64);
    }
    eprintln!();
...
fn delta_build<C: Codec>(bencher: Bencher, dist: &str) {
    ...
    bencher
        .counter(ItemsCount::new(N))
        .bench(|| black_box(&vals).iter().copied().collect::<DeltaColumn<u64, C>>());
}
