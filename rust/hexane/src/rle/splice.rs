...
mod partition_tests {
    ...
    fn state_item_count<T: RleValue, V: AsColumnRef<T>, C: Codec>(state: &RleState<'_, T, V, C>) -> usize {
        match state {
            RleState::Empty(_) => 0,
            RleState::Lone(_) => 1,
            RleState::Run(n, _) => *n,
...
pub(crate) fn tail<T: RleValue, C: Codec>(data: &[u8], tail: RleTail) -> (RleState<'_, T, T, C>, usize, usize) {
    let len = data.len();
