...
impl<'a, T: RleValue, C: Codec> RleLoadIter<'a, T, C> {
    ...
    pub fn try_next_run(&mut self) -> Result<Option<crate::Run<T::Get<'a>>>, PackError> {
        loop {
            ...
            let out = self.cut.track::<T, C>(segment);
...
mod tests {
    ...
    fn check_validate2<T: RleValue + ColumnValueRef<Encoding<Leb128> = RleEncoding<T>>>(data: &[u8]) {
