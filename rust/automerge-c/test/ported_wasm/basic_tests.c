...
static void test_create_clone_and_free(void** state) {
    ...
    AMdoc* const doc2 = AMitemToDoc(AMstackItem(stack_ptr, AMclone(doc1), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
}
...
static void test_should_be_able_to_set_and_get_a_simple_value(void** state) {
    ...
    AMdoc* const doc =
        AMitemToDoc(AMstackItem(stack_ptr,
                                AMcreate(AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromStr(AMstr("aabbcc")),
    ...
    assert_int_equal(AMitemToUint(AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("number1"), NULL), cmocka_cb,
    ...
    assert_int_equal(AMitemToInt(AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("number2"), NULL), cmocka_cb,
                                             AMexpect(AM_VAL_TYPE_INT))),
                     5);
    /*                                                                       */
    /* result = doc.getWithType(root, "number3")                             */
    /* assert.deepEqual(result, ["f64", 5.5])                                */
    assert_float_equal(AMitemToF64(AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("number3"), NULL), cmocka_cb,
                                               AMexpect(AM_VAL_TYPE_F64))),
                       5.5, DBL_EPSILON);
    /*                                                                       */
    /* result = doc.getWithType(root, "number4")                             */
    /* assert.deepEqual(result, ["f64", 5.5])                                */
    assert_float_equal(AMitemToF64(AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("number4"), NULL), cmocka_cb,
                                               AMexpect(AM_VAL_TYPE_F64))),
                       5.5, DBL_EPSILON);
    /*                                                                       */
    /* result = doc.getWithType(root, "number5")                             */
    /* assert.deepEqual(result, ["int", 5])                                  */
    assert_int_equal(AMitemToInt(AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("number5"), NULL), cmocka_cb,
                                             AMexpect(AM_VAL_TYPE_INT))),
                     5);
    /*                                                                       */
    ...
    assert_int_equal(AMitemToTimestamp(AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("time1"), NULL), cmocka_cb,
                                                   AMexpect(AM_VAL_TYPE_TIMESTAMP))),
                     1000);
    /*                                                                       */
    /* result = doc.getWithType(root, "time2")                               */
    /* assert.deepEqual(result, ["timestamp", new Date(1001)])               */
    assert_int_equal(AMitemToTimestamp(AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("time2"), NULL), cmocka_cb,
                                                   AMexpect(AM_VAL_TYPE_TIMESTAMP))),
                     1001);
    /*                                                                       */
...
static void test_should_be_able_to_make_subobjects(void** state) {
    ...
    assert_int_equal(AMitemToUint(AMstackItem(stack_ptr, AMmapGet(doc, submap, AMstr("number"), NULL), cmocka_cb,
                                              AMexpect(AM_VAL_TYPE_UINT))),
                     6);
}
...
static void test_should_be_able_to_make_lists(void** state) {
    ...
    AMbyteSpan str =
        AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, sublist, 0, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "z", str.count);
    /* assert.deepEqual(doc.getWithType(sublist, 1), ["str", "a"])           */
    str = AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, sublist, 1, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "a", str.count);
    /* assert.deepEqual(doc.getWithType(sublist, 2), ["str", "b"])           */
    str = AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, sublist, 2, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "b", str.count);
    /* assert.deepEqual(doc.getWithType(sublist, 3), ["str", "c"])           */
    str = AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, sublist, 3, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
...
static void test_should_be_able_to_use_counters(void** state) {
    ...
    assert_int_equal(AMitemToCounter(AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("counter"), NULL), cmocka_cb,
...
static void test_should_be_able_to_splice_text(void** state) {
    ...
    AMbyteSpan str =
        AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, text, 0, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "h", str.count);
    /* assert.deepEqual(doc.getWithType(text, 1), ["str", "e"])              */
    str = AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, text, 1, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "e", str.count);
    /* assert.deepEqual(doc.getWithType(text, 9), ["str", "l"])              */
    str = AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, text, 9, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "l", str.count);
    /* assert.deepEqual(doc.getWithType(text, 10), ["str", "d"])             */
    str = AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, text, 10, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "d", str.count);
    /* assert.deepEqual(doc.getWithType(text, 11), ["str", "!"])             */
    str = AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, text, 11, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "!", str.count);
    /* assert.deepEqual(doc.getWithType(text, 12), ["str", "?"])             */
    str = AMitemToStr(AMstackItem(stack_ptr, AMlistGet(doc, text, 12, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, 1);
    assert_memory_equal(str.src, "?", str.count);
}
...
static void test_should_be_able_to_save_all_or_incrementally(void** state) {
    ...
    AMbyteSpan const save1 = AMitemToBytes(AMstackItem(stack_ptr, AMsave(doc), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    /*                                                                       */
    /* doc.put("_root", "bar", 2)                                            */
    ...
    AMbyteSpan const saveMidway = AMitemToBytes(AMstackItem(
        stack_ptr, AMsave(AMitemToDoc(AMstackItem(stack_ptr, AMclone(doc), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)))),
        cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    /*                                                                       */
    /* const save2 = doc.saveIncremental();                                  */
    AMbyteSpan const save2 =
        AMitemToBytes(AMstackItem(stack_ptr, AMsaveIncremental(doc), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    /*                                                                       */
    /* doc.put("_root", "baz", 3);                                           */
    ...
    AMbyteSpan const saveA = AMitemToBytes(AMstackItem(stack_ptr, AMsave(doc), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    ...
    AMdoc* const docA =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(saveA.src, saveA.count), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    /* const docB = load(saveB);                                             */
    AMdoc* const docB =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(saveB_src, saveB_count), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    test_free(saveB_src);
    /* const docC = load(saveMidway)                                         */
    AMdoc* const docC = AMitemToDoc(
        AMstackItem(stack_ptr, AMload(saveMidway.src, saveMidway.count), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    ...
    AMbyteSpan const save = AMitemToBytes(AMstackItem(stack_ptr, AMsave(docA), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    assert_memory_equal(save.src,
                        AMitemToBytes(AMstackItem(stack_ptr, AMsave(docB), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES))).src,
                        save.count);
    /* assert.deepEqual(docA.save(), docC.save());                           */
    assert_memory_equal(save.src,
                        AMitemToBytes(AMstackItem(stack_ptr, AMsave(docC), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES))).src,
                        save.count);
}
...
static void test_should_be_able_to_splice_text_2(void** state) {
    ...
    AMbyteSpan str = AMitemToStr(AMstackItem(stack_ptr, AMtext(doc, text, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    ...
    str = AMitemToStr(AMstackItem(stack_ptr, AMtext(doc, text, &hash1), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    ...
    str = AMitemToStr(AMstackItem(stack_ptr, AMtext(doc, text, &hash2), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, strlen("hello big bad world"));
    assert_memory_equal(str.src, "hello big bad world", str.count);
...
static void test_local_inc_increments_all_visible_counters_in_a_map(void** state) {
    ...
    AMdoc* const doc1 =
        AMitemToDoc(AMstackItem(stack_ptr,
                                AMcreate(AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromStr(AMstr("aaaa")),
                                                                     cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)))),
                                cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    ...
    AMbyteSpan const save = AMitemToBytes(AMstackItem(stack_ptr, AMsave(doc1), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    AMdoc* const doc2 =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(save.src, save.count), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    AMstackItem(NULL,
                AMsetActorId(doc2, AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromStr(AMstr("bbbb")), cmocka_cb,
    ...
    AMdoc* const doc3 =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(save.src, save.count), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    AMstackItem(NULL,
                AMsetActorId(doc3, AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromStr(AMstr("cccc")), cmocka_cb,
                                                               AMexpect(AM_VAL_TYPE_ACTOR_ID)))),
                cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    ...
    AMbyteSpan const save1 =
        AMitemToBytes(AMstackItem(stack_ptr, AMsave(doc1), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    /* const doc4 = load(save1)                                              */
    AMdoc* const doc4 =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(save1.src, save1.count), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    /* assert.deepEqual(doc4.save(), save1);                                 */
    assert_memory_equal(AMitemToBytes(AMstackItem(stack_ptr, AMsave(doc4), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES))).src,
...
static void test_should_be_able_to_fetch_changes_by_hash(void** state) {
    ...
    AMdoc* const doc2 =
        AMitemToDoc(AMstackItem(stack_ptr,
                                AMcreate(AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromStr(AMstr("bbbb")),
                                                                     cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)))),
                                cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    /* doc1.put("/", "a", "b")                                               */
    ...
    AMitems change1 = AMstackItems(stack_ptr, AMgetChangeByHash(doc1, change_hash1.src, change_hash1.count), cmocka_cb,
    ...
    assert_memory_equal(AMchangeHash(AMitemToChange(AMitemsNext(&change1, 1))).src, change_hash1.src,
                        change_hash1.count);
}
...
static void test_recursive_sets_are_possible(void** state) {
    ...
    AMbyteSpan str = AMitemToStr(
        AMstackItem(stack_ptr, AMtext(doc, AMitemObjId(doc_item), NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, strlen("hello world"));
    assert_memory_equal(str.src, "hello world", str.count);
    ...
    str = AMitemToStr(AMstackItem(stack_ptr, AMtext(doc, l4, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    assert_int_equal(str.count, strlen("hello world"));
    assert_memory_equal(str.src, "hello world", str.count);
}
...
static void test_objects_without_properties_are_preserved(void** state) {
    ...
    AMbyteSpan const saved =
        AMitemToBytes(AMstackItem(stack_ptr, AMsave(doc1), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    /* const doc2 = load(saved);                                             */
    AMdoc* const doc2 =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(saved.src, saved.count), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
...
static void test_should_allow_you_to_forkAt_a_heads(void** state) {
    ...
    AMdoc* const A =
        AMitemToDoc(AMstackItem(stack_ptr,
                                AMcreate(AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromStr(AMstr("aaaaaa")),
                                                                     cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)))),
                                cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    /* A.put("/", "key1", "val1");                                           */
    ...
    AMdoc* const B = AMitemToDoc(AMstackItem(stack_ptr, AMfork(A, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    AMstackItem(NULL,
                AMsetActorId(B, AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromStr(AMstr("bbbbbb")), cmocka_cb,
                                                            AMexpect(AM_VAL_TYPE_ACTOR_ID)))),
                cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    /* A.put("/", "key3", "val3");                                           */
    ...
    AMitems AforkAt1_items = AMstackItems(
        stack_ptr,
        AMmapRange(AMitemToDoc(AMstackItem(stack_ptr, AMfork(A, &heads1), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC))),
    ...
    AMitems AforkAt2_items = AMstackItems(
        stack_ptr,
        AMmapRange(AMitemToDoc(AMstackItem(stack_ptr, AMfork(A, &heads2), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC))),
                   AM_ROOT, AMstr(NULL), AMstr(NULL), NULL),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
...
static void test_should_handle_merging_text_conflicts_then_saving_and_loading(void** state) {
    ...
    AMbyteSpan str =
        AMitemToStr(AMstackItem(stack_ptr,
                                AMtext(B,
                                       ...
                                       NULL),
                                cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    AMbyteSpan const str2 =
        AMitemToStr(AMstackItem(stack_ptr, AMtext(A, At, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
    ...
    /*                                                                       */
    /* const binary = A.save()                                               */
    AMbyteSpan const binary = AMitemToBytes(AMstackItem(stack_ptr, AMsave(A), cmocka_cb, AMexpect(AM_VAL_TYPE_BYTES)));
    /*                                                                       */
    /* const C = load(binary)                                                */
    AMdoc* const C =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(binary.src, binary.count), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    /*                                                                       */
    ...
    str = AMitemToStr(AMstackItem(stack_ptr, AMtext(C, At, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
