...
static void test_AMmapIncrement(void** state) {
    ...
    AMstackItem(NULL,
               AMmapPutCounter(doc_state->doc, AM_ROOT, AMstr("Counter"), 0),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    assert_int_equal(
        AMitemToCounter(AMstackItem(
            stack_ptr,
            AMmapGet(doc_state->doc, AM_ROOT, AMstr("Counter"), NULL),
            cmocka_cb, AMexpect(AM_VAL_TYPE_COUNTER))),
        0);
    AMfree(AMstackPop(stack_ptr, NULL));
    AMstackItem(NULL,
               AMmapIncrement(doc_state->doc, AM_ROOT, AMstr("Counter"), 3),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    assert_int_equal(
        AMitemToCounter(AMstackItem(
            stack_ptr,
...
#define static_void_test_AMmapPut(suffix, scalar_value)                      \
    static void test_AMmapPut##suffix(void** state) {                        \
        ...
        AMstack** stack_ptr = &doc_state->base_state->stack;           \
        AMstackItem(NULL,                                                     \
                   AMmapPut##suffix(doc_state->doc, AM_ROOT, AMstr(#suffix), \
                                    scalar_value),                           \
                   cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));                        \
        assert_true(                                                         \
            AMitemTo##suffix(AMstackItem(                                     \
                stack_ptr,                                                   \
                AMmapGet(doc_state->doc, AM_ROOT, AMstr(#suffix), NULL),     \
...
static void test_AMmapPutBytes(void** state) {
    ...
    AMstack** stack_ptr = &doc_state->base_state->stack;
    AMstackItem(NULL,
               AMmapPutBytes(doc_state->doc, AM_ROOT, KEY,
                             AMbytes(BYTES_VALUE, BYTES_SIZE)),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMbyteSpan const bytes = AMitemToBytes(
        AMstackItem(stack_ptr, AMmapGet(doc_state->doc, AM_ROOT, KEY, NULL),
...
static void test_AMmapPutNull(void** state) {
    ...
    DocState* doc_state = *state;
    AMstack** stack_ptr = &doc_state->base_state->stack;
    AMstackItem(NULL, AMmapPutNull(doc_state->doc, AM_ROOT, KEY), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMresult* result = AMstackResult(stack_ptr, AMmapGet(doc_state->doc, AM_ROOT, KEY, NULL), NULL,
               NULL);
...
#define static_void_test_AMmapPutObject(label)                                \
    static void test_AMmapPutObject_##label(void** state) {                   \
        ...
        AMobjId const* const obj_id = AMitemObjId(AMstackItem(                 \
            stack_ptr,                                                        \
            AMmapPutObject(doc_state->doc, AM_ROOT, AMstr(#label), obj_type), \
            cmocka_cb, AMexpect(AM_VAL_TYPE_VOID)));                               \
...
static void test_AMmapPutStr(void** state) {
    ...
    AMstack** stack_ptr = &doc_state->base_state->stack;
    AMstackItem(NULL,
               AMmapPutStr(doc_state->doc, AM_ROOT, AMstr("Str"),
                           AMstr("Hello, world!")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMbyteSpan const str = AMitemToStr(AMstackItem(
        stack_ptr, AMmapGet(doc_state->doc, AM_ROOT, AMstr("Str"), NULL),
...
static void test_get_NUL_key(void** state) {
    ...
    */
    ...
    static AMbyteSpan const OOPS_KEY = {
        .src = OOPS_SRC, .count = sizeof(OOPS_SRC) / sizeof(uint8_t)};

    static uint8_t const SAVED_DOC[] = {
        133, 111, 74,  131, 233, 150, 60,  244, 0,   116, 1,   16,  223, 253,
        146, 193, 58,  122, 66,  134, 151, 225, 210, 51,  58,  86,  247, 8,
        1,   49,  118, 234, 228, 42,  116, 171, 13,  164, 99,  244, 27,  19,
        150, 44,  201, 136, 222, 219, 90,  246, 226, 123, 77,  120, 157, 155,
        55,  182, 2,   178, 64,  6,   1,   2,   3,   2,   19,  2,   35,  2,
        64,  2,   86,  2,   8,   21,  6,   33,  2,   35,  2,   52,  1,   66,
        2,   86,  2,   87,  4,   128, 1,   2,   127, 0,   127, 1,   127, 1,
        127, 0,   127, 0,   127, 7,   127, 4,   111, 0,   112, 115, 127, 0,
        127, 1,   1,   127, 1,   127, 70,  111, 111, 112, 115, 127, 0,   0};
    ...
    AMdoc* const doc =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(SAVED_DOC, SAVED_DOC_SIZE),
                               cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    AMbyteSpan const str = AMitemToStr(
        AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, OOPS_KEY, NULL), cmocka_cb,
...
static void test_get_NUL_string_value(void** state) {
    ...
    */
    ...
    static uint8_t const SAVED_DOC[] = {
        133, 111, 74,  131, 63,  94,  151, 29,  0,   116, 1,   16,  156, 159,
        189, 12,  125, 55,  71,  154, 136, 104, 237, 186, 45,  224, 32,  22,
        1,   36,  163, 164, 222, 81,  42,  1,   247, 231, 156, 54,  222, 76,
        6,   109, 18,  172, 75,  36,  118, 120, 68,  73,  87,  186, 230, 127,
        68,  19,  81,  149, 185, 6,   1,   2,   3,   2,   19,  2,   35,  2,
        64,  2,   86,  2,   8,   21,  6,   33,  2,   35,  2,   52,  1,   66,
        2,   86,  2,   87,  4,   128, 1,   2,   127, 0,   127, 1,   127, 1,
        127, 0,   127, 0,   127, 7,   127, 4,   111, 111, 112, 115, 127, 0,
        127, 1,   1,   127, 1,   127, 70,  111, 0,   112, 115, 127, 0,   0};
    static size_t const SAVED_DOC_SIZE = sizeof(SAVED_DOC) / sizeof(uint8_t);
    ...
    AMdoc* const doc =
        AMitemToDoc(AMstackItem(stack_ptr, AMload(SAVED_DOC, SAVED_DOC_SIZE),
                               cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    AMbyteSpan const str = AMitemToStr(
        AMstackItem(stack_ptr, AMmapGet(doc, AM_ROOT, AMstr("oops"), NULL),
                   cmocka_cb, AMexpect(AM_VAL_TYPE_STR)));
...
static void test_range_iter_map(void** state) {
    BaseState* base_state = *state;
    AMstack** stack_ptr = &base_state->stack;
    AMdoc* const doc = AMitemToDoc(
        AMstackItem(stack_ptr, AMcreate(NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    AMstackItem(NULL, AMmapPutUint(doc, AM_ROOT, AMstr("a"), 3), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutUint(doc, AM_ROOT, AMstr("b"), 4), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutUint(doc, AM_ROOT, AMstr("c"), 5), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutUint(doc, AM_ROOT, AMstr("d"), 6), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMcommit(doc, AMstr(NULL), NULL), cmocka_cb,
               AMexpect(AM_VAL_TYPE_CHANGE_HASH));
    AMstackItem(NULL, AMmapPutUint(doc, AM_ROOT, AMstr("a"), 7), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMcommit(doc, AMstr(NULL), NULL), cmocka_cb,
               AMexpect(AM_VAL_TYPE_CHANGE_HASH));
    AMstackItem(NULL, AMmapPutUint(doc, AM_ROOT, AMstr("a"), 8), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutUint(doc, AM_ROOT, AMstr("d"), 9), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMcommit(doc, AMstr(NULL), NULL), cmocka_cb,
               AMexpect(AM_VAL_TYPE_CHANGE_HASH));
    AMactorId const* const actor_id = AMitemToActorId(AMstackItem(
        stack_ptr, AMgetActorId(doc), cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)));
    ...
    /* ["b"-"d") */
    AMitems range = AMstackItems(
        stack_ptr, AMmapRange(doc, AM_ROOT, AMstr("b"), AMstr("d"), NULL),
        cmocka_cb, AMexpect(AM_VAL_TYPE_UINT));
    /* First */
...
static void test_map_range_back_and_forth_single(void** state) {
    ...
    AMstackItem(NULL, AMmapPutStr(doc, AM_ROOT, AMstr("1"), AMstr("a")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc, AM_ROOT, AMstr("2"), AMstr("b")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc, AM_ROOT, AMstr("3"), AMstr("c")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));

    /* Forward, back, back. */
    AMitems range_all = AMstackItems(
        stack_ptr, AMmapRange(doc, AM_ROOT, AMstr(NULL), AMstr(NULL), NULL),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    /* First */
    ...
    assert_int_equal(AMactorIdCmp(AMobjIdActorId(next_back_obj_id), actor_id),
                     0);
...
static void test_map_range_back_and_forth_double(void** state) {
    ...
    AMactorId const* const actor_id1 =
        AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromBytes("\0", 1),
                                   cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)));
    AMstackItem(NULL, AMsetActorId(doc1, actor_id1), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));

    AMstackItem(NULL, AMmapPutStr(doc1, AM_ROOT, AMstr("1"), AMstr("a")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc1, AM_ROOT, AMstr("2"), AMstr("b")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc1, AM_ROOT, AMstr("3"), AMstr("c")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));

    /* The second actor should win all conflicts here. */
    AMdoc* const doc2 = AMitemToDoc(
        AMstackItem(stack_ptr, AMcreate(NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    AMactorId const* const actor_id2 =
        AMitemToActorId(AMstackItem(stack_ptr, AMactorIdFromBytes("\1", 1),
                                   cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)));
    AMstackItem(NULL, AMsetActorId(doc2, actor_id2), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc2, AM_ROOT, AMstr("1"), AMstr("aa")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc2, AM_ROOT, AMstr("2"), AMstr("bb")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc2, AM_ROOT, AMstr("3"), AMstr("cc")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));

    AMstackItem(NULL, AMmerge(doc1, doc2), cmocka_cb,
               AMexpect(AM_VAL_TYPE_CHANGE_HASH));

    /* Forward, back, back. */
    AMitems range_all = AMstackItems(
        stack_ptr, AMmapRange(doc1, AM_ROOT, AMstr(NULL), AMstr(NULL), NULL),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    /* First */
    AMitem* next = AMitemsNext(&range_all, 1);
    assert_non_null(next);
    ...
    AMobjId const* next_back_obj_id = AMitemObjId(next_back);
    assert_int_equal(AMobjIdCounter(next_back_obj_id), 3);
    assert_int_equal(AMactorIdCmp(AMobjIdActorId(next_back_obj_id), actor_id2),
                     0);
...
static void test_map_range_at_back_and_forth_double(void** state) {
    ...
    AMstackItem(NULL, AMmerge(doc1, doc2), cmocka_cb,
               AMexpect(AM_VAL_TYPE_CHANGE_HASH));
    AMitems const heads = AMstackItems(stack_ptr, AMgetHeads(doc1), cmocka_cb,
                                      AMexpect(AM_VAL_TYPE_CHANGE_HASH));

    /* Forward, back, back. */
    AMitems range_all = AMstackItems(
        stack_ptr, AMmapRange(doc1, AM_ROOT, AMstr(NULL), AMstr(NULL), &heads),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    /* First */
    AMitem* next = AMitemsNext(&range_all, 1);
    assert_non_null(next);
...
static void test_get_range_values(void** state) {
    ...
    AMdoc* const doc1 = AMitemToDoc(
        AMstackItem(stack_ptr, AMcreate(NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));
    AMstackItem(NULL, AMmapPutStr(doc1, AM_ROOT, AMstr("aa"), AMstr("aaa")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc1, AM_ROOT, AMstr("bb"), AMstr("bbb")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc1, AM_ROOT, AMstr("cc"), AMstr("ccc")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutStr(doc1, AM_ROOT, AMstr("dd"), AMstr("ddd")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMcommit(doc1, AMstr(NULL), NULL), cmocka_cb,
               AMexpect(AM_VAL_TYPE_CHANGE_HASH));

    AMitems const v1 = AMstackItems(stack_ptr, AMgetHeads(doc1), cmocka_cb,
                                   AMexpect(AM_VAL_TYPE_CHANGE_HASH));
    AMdoc* const doc2 = AMitemToDoc(AMstackItem(
        stack_ptr, AMfork(doc1, NULL), cmocka_cb, AMexpect(AM_VAL_TYPE_DOC)));

    AMstackItem(NULL, AMmapPutStr(doc1, AM_ROOT, AMstr("cc"), AMstr("ccc V2")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMcommit(doc1, AMstr(NULL), NULL), cmocka_cb,
               AMexpect(AM_VAL_TYPE_CHANGE_HASH));

    AMstackItem(NULL, AMmapPutStr(doc2, AM_ROOT, AMstr("cc"), AMstr("ccc V3")),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMcommit(doc2, AMstr(NULL), NULL), cmocka_cb,
               AMexpect(AM_VAL_TYPE_CHANGE_HASH));
    ...
    for (item1 = NULL, item_back1 = NULL, count = 0; item1 && item_back1;
         item1 = AMitemsNext(&range, 1),
        item_back1 = AMitemsNext(&range_back, 1), ++count) {
        ...
        if ((count == middle) && (middle & 1)) {
            ...
            assert_true(
                AMobjIdEqual(AMitemObjId(item1), AMitemObjId(item_back1)));
        } else {
            assert_false(indices_match);
        }
        AMitem* item2 = AMstackItem(
            stack_ptr, AMmapGet(doc1, AM_ROOT, AMitemKey(item1), NULL), NULL, NULL);
        AMitem* item_back2 = AMstackItem(
            stack_ptr, AMmapGet(doc1, AM_ROOT, AMitemKey(item_back1), NULL), NULL,
            NULL);
        ...
        assert_true(AMitemEqual(item1, item2));
        assert_true(
            AMobjIdEqual(AMitemObjId(item_back1), AMitemObjId(item_back2)));
        AMfree(AMstackPop(stack_ptr, NULL));
    }

    /* Forward vs. reverse: partial current map range. */
    range = AMstackItems(
        stack_ptr, AMmapRange(doc1, AM_ROOT, AMstr("aa"), AMstr("dd"), NULL),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    ...
    range = AMstackItems(
        stack_ptr, AMmapRange(doc1, AM_ROOT, AMstr(NULL), AMstr(NULL), &v1),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    size = AMitemsSize(&range);
    assert_int_equal(size, 4);
    range_back = AMitemsReversed(&range);
    ...
    for (item1 = NULL, item_back1 = NULL, count = 0; item1 && item_back1;
        ...
        AMitem* item2 = AMstackItem(
            stack_ptr, AMmapGet(doc1, AM_ROOT, AMitemKey(item1), &v1), NULL, NULL);
        AMitem* item_back2 =
            AMstackItem(stack_ptr, AMmapGet(doc1, AM_ROOT, AMitemKey(item_back1), &v1),
                       NULL, NULL);
        /** \note An item returned from an `AM...Get()` call doesn't include the
                  index used to retrieve it. */
        assert_false(AMitemIdxType(item2));
    ...
    range = AMstackItems(
        stack_ptr, AMmapRange(doc1, AM_ROOT, AMstr("bb"), AMstr(NULL), &v1),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    size = AMitemsSize(&range);
    assert_int_equal(size, 3);
    range_back = AMitemsReversed(&range);
    ...
    range = AMstackItems(
        stack_ptr, AMmapRange(doc1, AM_ROOT, AMstr(NULL), AMstr(NULL), NULL),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    AMitems obj_items = AMstackItems(stack_ptr, AMobjItems(doc1, AM_ROOT, NULL),
                                    cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    ...
    range = AMstackItems(
        stack_ptr, AMmapRange(doc1, AM_ROOT, AMstr(NULL), AMstr(NULL), &v1),
        cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    obj_items = AMstackItems(stack_ptr, AMobjItems(doc1, AM_ROOT, &v1),
                            cmocka_cb, AMexpect(AM_VAL_TYPE_STR));
    assert_int_equal(AMitemsSize(&range), AMitemsSize(&obj_items));
...
int run_map_tests(void) {
    const struct CMUnitTest tests[] = {
        ...
        cmocka_unit_test_setup_teardown(test_get_NUL_key, setup_base,
                                        teardown_base),
        cmocka_unit_test_setup_teardown(test_get_NUL_string_value, setup_base,
                                        teardown_base),
        cmocka_unit_test_setup_teardown(test_range_iter_map, setup_base,
                                        teardown_base),
        cmocka_unit_test_setup_teardown(test_map_range_back_and_forth_single,
                                        setup_base, teardown_base),
        cmocka_unit_test_setup_teardown(test_map_range_back_and_forth_double,
                                        setup_base, teardown_base),
        cmocka_unit_test_setup_teardown(test_map_range_at_back_and_forth_single,
                                        setup_base, teardown_base),
        cmocka_unit_test_setup_teardown(test_map_range_at_back_and_forth_double,
                                        setup_base, teardown_base),
        cmocka_unit_test_setup_teardown(test_get_range_values, setup_base,
                                        teardown_base),
    };
