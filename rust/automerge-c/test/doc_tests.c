...
static int setup(void** state) {
    ...
    hex_to_bytes(test_state->actor_id_str.src, test_state->actor_id_bytes,
...
static void test_AMkeys_empty(void** state) {
    ...
    AMdoc* const doc =
        AMitemToDoc(AMstackItem(stack_ptr, AMcreate(NULL), cmocka_cb,
                               AMexpect(AM_VAL_TYPE_DOC)));
    AMitems forward =
        AMstackItems(stack_ptr, AMkeys(doc, AM_ROOT, NULL), cmocka_cb,
...
static void test_AMkeys_list(void** state) {
    ...
    AMstackItem(NULL, AMlistPutInt(doc, list, 0, true, 0), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMlistPutInt(doc, list, 1, true, 0), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMlistPutInt(doc, list, 2, true, 0), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMitems forward = AMstackItems(stack_ptr, AMkeys(doc, list, NULL), cmocka_cb,
...
static void test_AMkeys_map(void** state) {
    ...
    AMdoc* const doc =
        AMitemToDoc(AMstackItem(stack_ptr, AMcreate(NULL), cmocka_cb,
                               AMexpect(AM_VAL_TYPE_DOC)));
    AMstackItem(NULL, AMmapPutInt(doc, AM_ROOT, AMstr("one"), 1), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutInt(doc, AM_ROOT, AMstr("two"), 2), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMmapPutInt(doc, AM_ROOT, AMstr("three"), 3), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMitems forward =
        AMstackItems(stack_ptr, AMkeys(doc, AM_ROOT, NULL), cmocka_cb,
                    AMexpect(AM_VAL_TYPE_STR));
...
static void test_AMputActor_bytes(void** state) {
    ...
    AMactorId const* actor_id = AMitemToActorId(
        AMstackItem(stack_ptr,
                   AMactorIdFromBytes(test_state->actor_id_bytes,
                                      test_state->actor_id_size),
                   cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)));
    AMstackItem(NULL, AMsetActorId(test_state->doc_state->doc, actor_id),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    actor_id = AMitemToActorId(
        AMstackItem(stack_ptr, AMgetActorId(test_state->doc_state->doc),
                   cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)));
...
static void test_AMputActor_str(void** state) {
    ...
    AMstack** stack_ptr = &test_state->doc_state->base_state->stack;
    AMactorId const* actor_id = AMitemToActorId(
        AMstackItem(stack_ptr, AMactorIdFromStr(test_state->actor_id_str),
                   cmocka_cb, AMexpect(AM_VAL_TYPE_ACTOR_ID)));
    AMstackItem(NULL, AMsetActorId(test_state->doc_state->doc, actor_id),
               cmocka_cb, AMexpect(AM_VAL_TYPE_VOID));
    actor_id = AMitemToActorId(
...
static void test_AMspliceText(void** state) {
    ...
    AMstackItem(NULL, AMspliceText(doc, text, 0, 0, AMstr("one + ")), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMspliceText(doc, text, 4, 2, AMstr("two = ")), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
    AMstackItem(NULL, AMspliceText(doc, text, 8, 2, AMstr("three")), cmocka_cb,
               AMexpect(AM_VAL_TYPE_VOID));
