...
impl<'a, 'b> MutationTracker<'a, 'b> {
    ...
    fn diff_at_path<'c>(&'a self, path: &'c Path, subdiff: amb::Diff) -> Option<amb::Diff> {
        ...
        // the enclosing diffs
        enum Intermediate {
            Map(amb::ObjectID, MapType, String),
            Seq(amb::ObjectID, SequenceType, usize),
        };
        ...
        let diff = intermediates
            ...
            .rfold(subdiff, |diff_so_far, intermediate| match intermediate {
                Intermediate::Map(oid, map_type, k) => amb::Diff::Map(amb::MapDiff {
                    object_id: oid.to_string(),
                    obj_type: map_type.to_obj_type(),
                    props: hashmap! {k => hashmap!{random_op_id().to_string() => diff_so_far}},
                }),
                Intermediate::Seq(oid, seq_type, index) => amb::Diff::Seq(amb::SeqDiff {
                    object_id: oid.to_string(),
        ...
        Some(amb::Diff::Map(amb::MapDiff {
            object_id: amb::ObjectID::Root.to_string(),
...
pub(crate) fn resolve_path(
    path: &Path,
    objects: &HashMap<amb::ObjectID, Rc<Object>>,
) -> Option<Rc<Object>> {
    let mut stack = path.clone().0;
    stack.reverse();
