...
## B3 structural extension (40-minute bound, approved repair baseline)

- Add `prototype/structural_tests.rs`, registered in `prototype.rs`, with EX-08/09 list tests before removing list rejection in `prototype/core.rs`. Reuse real `doc.list_range_for`, `get_cursor_for`/recorded element keys, selected clock, endpoint transitions. Assert element anchors independently of replacement value IDs. Exercise earlier actor capture recompilation and selected index authoring.
- Add EX-10/11 counter tests before lifting increment/counter rejection; cover self-diff/duplicate/empty transitions, excluded base and mixed scalar/counter suppression. Fix only any evidenced paths in `op_set2` counter folding or `iter/{map_range,list_range}`.
- Add `prototype/format_observer.rs` (test only): per-character text plus mark maps, replay native `SpliceText` marks, `Mark`, deletes, object creation and block insertion. Compare replay to full real selected spans; never synthesize expected spans using production evaluator. Default encoding is pinned and checked; multi-width fixture follows if supported.
- Add EX-12/13/14 before lifting text/mark rejection. Use `spans_for`, `marks_for`, `text_for` with captured scope. Potential source fixes: `iter/spans.rs`, mark query scan gates, and `patches/patch_log.rs::ExposeQueue::flush_obj` for formatted restored subtrees. If changing exposure, cover ordinary control-free text with block marker and preserve block object contents.
- Extend EX-07 to formatted subtree restoration; restore complete subtree once with no duplicated child deltas. Add native direct/batch/merge/load characterization and capture reconstruction for supported families, without claiming selected optimized import logs or sync.
- Keep unproven families rejected if blocked. Log behavioral reds/greens as `extend-*`, then full feature/default suites and targeted rustfmt. Record exact fixture/test totals, LOC/files and debug timings; commit coherent checkpoint with jj.
...
- At B1, the session explicitly rejected lists, text, counters, marks, table creation and competing controls. B3 lifts the first four after red/green tests, retains table/competing-control rejection and adds list/text structural exposure fixes. Shared text exposure now iterates internal spans with mark state and real block IDs. Added-text coalescing compares complete after-format rather than mark delta. Ordinary control-free compatibility regressions accompany both changes.
