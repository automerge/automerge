...
pub use external_types::{
    ExternalElemId as ElemId, ExternalKey as Key, ExternalObjId as ObjId, ExternalOpId as OpId,
};
...
use types::{ElemId as InternalElemId, Key as InternalKey, ObjId as InternalObjId, Op};
...
use types::OpId as InternalOpId;
...
impl Automerge {
    ...
    // PropAt::()
    // NthAt::()
    fn export_prop(&self, key_index: usize) -> Option<String> {
        self.ops.m.borrow().props.get_safe(key_index).cloned()
    }
    ...
    pub fn keys<'a, O: Into<ObjId<'a>>>(&self, obj: O) -> Vec<String> {
        ...
        q.keys
            ...
            .filter_map(|k| match k {
                InternalKey::Map(key_index) => Some(self.export_prop(*key_index).unwrap()),
                InternalKey::Seq(_) => None,
            })
            .collect()
    }
    ...
    pub fn length<'a, O: Into<ObjId<'a>>>(&self, obj: O) -> usize {
        let obj = self.import_objid(obj.into());
        self.ops.search(obj, query::Len::new(obj)).len
    }
    ...
    pub fn set<'a, O: Into<ObjId<'a>>, P: Into<Prop>, V: Into<Value>>(
        ...
        let obj = self.import_objid(obj.into());
        self.local_op(obj, prop.into(), value.into())
    ...
    pub fn inc<'a, O: Into<ObjId<'a>>, P: Into<Prop>>(
        ...
        match self.local_op(self.import_objid(obj), prop.into(), OpType::Inc(value))? {
    ...
    fn import_ops(&mut self, change: &Change, change_id: usize) -> Vec<Op> {
        change
            ...
            .map(|(i, c)| {
                ...
                let obj: InternalObjId = self.import_objid(&c.obj);
                let pred = c
                    .pred
                    .iter()
                    .map(|i| self.import_opid(&OpId::from(i)))
                    .collect();
                let key = match &c.key {
                    legacy::Key::Map(n) => {
                        InternalKey::Map(self.ops.m.borrow_mut().props.cache(n.to_string()))
                    }
                    legacy::Key::Seq(legacy::ElementId::Head) => {
                        InternalKey::Seq(InternalElemId::Head)
                    }
                    // FIXME dont need to_string()
                    legacy::Key::Seq(legacy::ElementId::Id(i)) => {
                        InternalKey::Seq(self.import_opid(&OpId::from(i)).into())
                    }
                };
                Op {
                    change: change_id,
    ...
    fn export_opid(&self, opid: &InternalOpId) -> Option<OpId> {
        self.ops.m.borrow().export_opid(opid)
    }

    fn export_objid(&self, objid: &InternalObjId) -> Option<ObjId<'static>> {
        self.ops.m.borrow().export_objid(objid)
    }

    fn import_objid<'a, A: Into<ObjId<'a>>>(&self, objid: A) -> InternalObjId {
        self.ops.m.borrow_mut().import_objid(objid)
    }

    fn export_key(&self, key: &InternalKey) -> Option<Key<'static>> {
        self.ops.m.borrow().export_key(key)
    }
    ...
    pub fn dump(&self) {
        ...
        for i in self.ops.iter() {
            let id = self.export_opid(&i.id).unwrap();
            let obj = self.export_objid(&i.obj).unwrap();
            let key = self.export_key(&i.key).unwrap().to_string();
            ...
            let pred: Vec<_> = i
                .pred
                .iter()
                .map(|id| self.export_opid(id).unwrap())
                .collect();
            let succ: Vec<_> = i
                .succ
                .iter()
...
mod tests {
    ...
    fn test_list() -> Result<(), AutomergeError> {
        let mut doc = Automerge::new();
        doc.set_actor(ActorId::random());
        let list_id = doc.set(ObjId::Root, "items", Value::list())?.unwrap();
