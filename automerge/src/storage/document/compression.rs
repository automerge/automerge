...
use crate::storage::{column_layout::compression, shift_range, ChunkType, Header, RawColumns};
...
pub(super) struct Args<'a, T: compression::ColumnCompression, DirArgs> {
    ...
    /// Arguments specific to the direction (compression or uncompression)
    pub(super) dir_args: DirArgs,
}
...
pub(super) fn compress<'a>(args: Args<'a, compression::Uncompressed, CompressArgs>) -> Vec<u8> {
    let header_len = args.dir_args.original_header_len;
    let threshold = args.dir_args.threshold;
