...
use crate::{
    ...
    types::{ChangeHash, ElemId, Key, ObjId, ObjType, Op, OpId, OpType},
    Change, ScalarValue,
};
...
pub(crate) fn reconstruct_document<'a, O: DocObserver>(
    ...
    for op_res in doc.iter_ops() {
        ...
        if obj == current_object.id {
            ...
            if obj < current_object.id {
                ...
                let loaded = current_object.finish(&mut collector)?;
    ...
    Ok(Reconstructed {
        result,
        changes: history.into_iter().map(crate::Change::new).collect(),
        heads,
        max_op,
    })
}
...
impl LoadingObject {
    ...
    fn finish(mut self, collector: &mut ChangeCollector<'_>) -> Result<LoadedObject, Error> {
        let mut ops = Vec::new();
