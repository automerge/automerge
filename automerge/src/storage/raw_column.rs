...
use super::column_layout::{compression, ColumnSpec};

/// This is a "raw" column in the sense that it is just the column specification[1] and range. This
/// is in contrast to [`Column`] which is aware of composite columns such as value columns[2] and
/// group columns[3].
///
/// `RawColumn` is generally an intermediary object which is parsed into a [`GenericColumnRange`].
///
    ...
    pub(crate) fn parse(input: &[u8]) -> parse::ParseResult<'_, Self> {
        ...
        if !are_normal_sorted(&columns) {
            return Err(parse::ParseError::Error(
                parse::ErrorKind::InvalidRawColumns,
            ));
        }
