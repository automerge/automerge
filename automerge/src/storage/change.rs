...
use super::{parse, shift_range, CheckSum, ChunkType, ColumnLayout, Header, RawColumns};
...
impl<'a> Change<'a, Unverified> {
    pub(crate) fn parse(input: &'a [u8]) -> parse::ParseResult<'_, Change<'a, Unverified>> {
        // TODO(alex): check chunk type
        let (_i, header) = Header::parse(input)?;
        Self::parse_from_header(input, header)
    }

    /// Parse a change chunk. `input` should be the entire chunk, including the header bytes.
    pub(crate) fn parse_from_header(
        input: &'a [u8],
        header: Header,
    ) -> parse::ParseResult<'_, Change<'a, Unverified>> {
        tracing::trace!(?input, ?header, "parsing from header");
        let (i, _) = parse::drop_n(header.data_bytes().start, input)?;
        let (i, deps) = parse::length_prefixed(parse::leb128_u64, parse::change_hash)(i)?;
        ...
        let ops_start = input.len() - i.len();
        // We don't actually need the data, we just need to make sure it's there.
        let (i, _) = parse::take_n(ops_meta.total_column_len(), i)?;
        let ops_data = ops_start..(ops_start + ops_meta.total_column_len());

        let extra_start = input.len() - i.len();
        let extra_bytes = extra_start..(extra_start + i.len());

        let ops_meta = ops_meta.uncompressed().ok_or(parse::ParseError::Error(
            parse::ErrorKind::CompressedChangeCols,
        ))?;
        let col_layout = ColumnLayout::parse(ops_data.len(), ops_meta.iter()).map_err(|e| {
            parse::ParseError::Error(parse::ErrorKind::InvalidColumns {
                columns_type: "change ops",
                description: e.to_string(),
            })
        })?;
        let ops_meta = ChangeOpsColumns::try_from(col_layout).map_err(|e| {
            parse::ParseError::Error(parse::ErrorKind::InvalidColumns {
                columns_type: "change ops",
                description: e.to_string(),
            })
...
impl ChangeBuilder<Set<NonZeroU64>, Set<ActorId>, Set<u64>, Set<i64>> {
    pub(crate) fn build<'a, A, I, O>(self, ops: I) -> Change<'static, Verified>
    where
