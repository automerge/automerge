...
use crate::{convert, indexed_cache::IndexedCache};
    ...
    I: Iterator<Item = C> + Clone + 'a,
{
    ...
    pub(crate) fn new(actor: A, ops: I) -> ChangeActors<'a, A, I, O, C> {
        ...
        let mut other_actors = ops.clone().fold(BTreeSet::new(), |mut acc, op| {
    ...
    fn translate_opid(&self, opid: &O) -> ChangeOpId {
        ChangeOpId {
            actor: self.index.lookup(opid.actor()).unwrap(),
