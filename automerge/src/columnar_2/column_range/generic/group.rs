...
use crate::columnar_2::{
    ...
    encoding::{DecodeColumnError, RleDecoder},
};
...
impl<'a> Iterator for GroupIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self.num.next() {
            None => None,
            Some(None) => Some(Err(DecodeColumnError::UnexpectedNull(
                "group num".to_string(),
            ))),
            Some(Some(num)) => {
                let mut row = Vec::new();
                            ...
                            Some(Err(e)) => {
                                let e = match e {
                                    DecodeColumnError::UnexpectedNull(col) => {
                                        DecodeColumnError::UnexpectedNull(format!(
                                            "grouped col {}:{}",
                                            index, col
                                        ))
                                    }
                                    DecodeColumnError::InvalidValue {
                                        ...
                                        column: format!("grouped col {}:{}", index, column),
                                        description,
                                    },
                                };
                                return Some(Err(e));
...
impl<'a> Iterator for GroupedColIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            Self::Value(viter) => viter.next().map(|v| v.map(SimpleValue::Value)),
            Self::Simple(siter) => siter.next().map(Ok),
        }
    }
}
